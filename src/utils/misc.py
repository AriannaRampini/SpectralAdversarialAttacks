...
#--------------------------------------------------------
def get_pytorch_tensors(vertices, faces, device):
    vertices_torch = torch.as_tensor(vertices, 
                                     device=device,
                                     dtype=torch.double).clone().detach().requires_grad_(True)
    faces_torch = torch.as_tensor(faces, device=device, dtype=torch.int64)
